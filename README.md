...
each reference with `MethodHandles.Lookup` on the new-side and old-side
classpaths. A verdict uika calls broken that the JVM links fine fails the run
as a false positive; ok/unknown verdicts that fail on the new side but linked
on the old side are listed as false-negative candidates for triage.
