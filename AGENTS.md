...
shares, the dump format, the JDK API layer, and runtime load evidence. This file holds what only
agents need: invariants, internal semantics, and lessons that must not be
relearned by experiment.

## Development

- Measure the jar the way the plugins run it: the launcher flags passed by hand
  and `-Duika.child=true`, or the number includes a second JVM boot. The
  uika-performance skill has the workloads and the expected numbers. A
  `gradle run` or a bare `-cp build/classes` is not a benchmark.
...
- Golden regression (`cli-java/src/test/java/net/exoego/uika/cli/GoldenTest.java`;
  the bless workflow is in CONTRIBUTING.md): golden inputs use paths relative
  to the test working directory (`GoldenTest.fixture`) so the JSON stays
  machine-independent; keep it that way. `Check.checkScanned` sorts violations
  by string value before returning, so the JSON report is canonical — without
...
- `make probe` (tools/jvm-probe/run-fixtures.sh, the jar with the relaunch off —
  verdicts are optimization-independent) resolves each distinct verdict record via
...
- `--jdk-release N` (check/upgrade-check; user-facing behaviour in README)
  ...
  (goldens). Because the SAME index sits in the old
  ...
  notes live in `cli-java/src/main/java/net/exoego/uika/cli/Jdk.java` (12+ has
  module dirs, 9-11 does not, 8 is unsupported; codes are base-36 and real files
  keep 6/7 in joint dirs).
...
- `--jdk-release-old N --jdk-release-new M` (check) makes the JDK upgrade the
  compared pair instead of layering one release under both sides. The argument
  parser (`Cli`) REJECTS `--old`/`--new` alongside it rather than accepting and
...
- The dump records `jdkRelease` (additive) TWICE: once per module, once for the dump.
  ...
  naming it sends `Jdk.releaseIndex` after a release ct.sym never carried.
- `upgrade-check` turns a before/after disagreement into extra runs appended by
  `Commands.planJdkRuns` with `jdkPair` set and both jar lists empty, ONE PER MODULE
  whose release moved, over that module's own classpath. Per module for two reasons. A
...
- A JDK run is named `:app (JDK 11 -> 17)`, module AND pair. That string is the key
  ...
  (`ModuleOutcome.jdkModules` and `jdkPair` carry what that section shows). A JDK run
  compares two releases of the JDK while a module row compares two versions of a jar,
  so side by side their broken counts read as parts of one total that does not add up.
...
- No object per class in anything proportional to the scan — int symbols
  (`Intern`) and shared arenas with range references (`IntArena`). The JVM-side
...
- Preserve determinism: output sorted by string value (`Intern.compare`, UTF-8
  ...
  `Reason`: order via `text`, never ordinal.
...
- Reachability edges are the one arena proportional to the whole scan that is
  not always built; keep them gated behind `collectEdges` (driven by app-root
...
README.md is the current answer, and a hand-kept table drifts.
