...
def _aspect_impl(target, ctx):
    ...
    # No build_outputs here, so a sweep always records the targets' own jars while the rule
    # can skip them. Not an oversight, and not fixable by an aspect parameter: `//...`
    # includes the uika_dump targets, whose `targets` attribute applies this same aspect with
    # the default value, so a parameterized command-line instance is a SECOND aspect over the
    # same targets and both declare `<name>.uika-manifest.tsv`. Bazel rejects that outright
    # ("file ... is generated by these conflicting actions"), measured on 9.2. Fixing it means
    # naming the fragment per parameterization and teaching the merge which variant to read,
    # which is more machinery than a whole-build baseline is worth today.
